...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (is (= 1.2 (:connector-plug-mass-kg (store/device-unit s "device-unit-1"))))
      (is (number? (:sim-peak-insertion-force-n (store/device-unit s "device-unit-1")))
          "seed data is genuinely simulation-derived (ADR-2607991500), not hand-typed")
      (is (< (:sim-peak-insertion-force-n (store/device-unit s "device-unit-1")) 30.0)
          "device-unit-1's connector-plug-mass-kg clears the real insertion-force ceiling with margin")
      (is (true? (:robotics-sim-verified? (store/device-unit s "device-unit-6"))) "seeded as already-on-file")
      (is (= 5.0 (:connector-plug-mass-kg (store/device-unit s "device-unit-6"))))
      (is (> (:sim-peak-insertion-force-n (store/device-unit s "device-unit-6")) 30.0)
          "device-unit-6's deliberately too-heavy connector-plug-mass-kg exceeds the real insertion-force ceiling")
      ...
      (is (= ["device-unit-1" "device-unit-2" "device-unit-3" "device-unit-4" "device-unit-5" "device-unit-6"]
