...
`cloud-itonami-isic-2620` is an OSS open-business blueprint for computer and
peripheral-equipment final-assembly manufacturing enablement, robotics-premised.

## Maintainers
Maintainers may merge changes that preserve these invariants:
- a robot action the governor refuses is never dispatched to hardware.
- the Assembly Governor remains independent of the advisor.
- hard policy violations (force-ship, record-suppression, unauthorized disclosure) cannot be overridden by human approval.
- every shipment, sign-off, record and disclose path is auditable.
