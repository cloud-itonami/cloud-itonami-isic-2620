(ns deviceassembly.operation
  "OperationActor -- one device-assembly operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Device Assembly Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Assembly Governor (:govern) and
  ...
  One graph run = one device-assembly operation (intake -> advise ->
  ...
  decision to a human operator (the compliance engineer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/ship-device-unit`/`:actuation/issue-declaration-of-
  conformity` ALWAYS reach this node when the governor is clean -- see
  `deviceassembly.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [deviceassembly.deviceassemblyadvisor :as deviceassemblyadvisor]
            [deviceassembly.governor :as governor]
...
(defn build
  ...
  `deviceassembly.store/Store`).
  opts:
    :advisor      -- a `deviceassembly.deviceassemblyadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (deviceassemblyadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Device Assembly Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (deviceassemblyadvisor/-advise advisor store request)]
            {:proposal p :audit [(deviceassemblyadvisor/trace request p)]})))

      ;; Assembly Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
